...
**Current version: Codex Galaxy 3.1.0**
...
3.1.0 keeps the existing API Audit button, layout, and workflow while adding three automatic GPT behavior-fingerprint probes in the background. Users do not copy prompts or paste answers. Reports still show one 100-point total with GPT fingerprint candidate and weight details, and the ranking service applies the same scoring. Only the aggregate candidate, probability, and valid-sample count are submitted; API keys and raw outputs never enter the ranking service.

3.0.5 fixes a startup rendering regression left by the 3.0.4 heading cleanup: Accounts no longer writes to the removed current-profile element, so existing local profiles and API rankings render normally again. The upgrade does not migrate or clear accounts, authentication, or history.

3.0.4 applies the final two-column hierarchy: the redundant Codex status card and the entire duplicate heading row are removed; the three runtime states sit directly above Accounts, while health sits directly above API Audit/Ranking at matching widths. The model-scope selector, sort selector, and ranking refresh button now sit beside the API title, reducing first-screen dead space.

3.0.3 completes the density correction: the four runtime states now live in their own compact grid beside a separate health card, so the grid no longer exposes a blank area stretched by the health panel; the page feature summary uses content width, and header controls no longer spread apart. The gray native health overlay and refresh-button overlap are fixed; Chinese component labels, hover reasons, the health guide, and foreground-only five-minute polling remain available.
...
2.2.4 fixes history replay when switching from DeepSeek V4 Pro back to the official GPT account; adds a 90-day color-coded OpenAI health timeline with hoverable public incident reasons, and exposes low, medium, and high reasoning levels for DeepSeek V4 Pro.

2.2.3 adds an OpenAI official status health widget and detail page, with a selectable home-pinned service component.

2.2.2 adds a prominent three-step recharge workflow before the plans, including a direct link to the CDK recharge page.

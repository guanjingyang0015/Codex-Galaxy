...
- 只保存 Base host、脱敏 API 路径、平台名、模型名、测试时间、聚合分数和脱敏测试指标。
...
- 服务端独立重算评分：GPT 为协议 25 分、模型声明 25 分、行为指纹 15 分、能力探针 15 分、稳定性 10 分、速度 10 分；非 GPT 沿用模型核对 40 分且不运行行为指纹。模型声明明确不匹配时总分最高 49 分，GPT 行为指纹候选不一致时总分最高 79 分。

...
- “API 检测”可以检测已保存的 API，也可以临时填写 Base URL、API Key 和模型；临时 Key 只在本机内存中使用，不会保存。
- 检测包含 `/models`、Responses 返回、模型是否列出、推理强度、响应速度和 usage 结构，并给出 100 分制的兼容性参考分。
- 只有勾选“将脱敏结果提交到公共排名”时才会上传结果；服务器不接收 API Key、请求正文、完整回答或聊天记录。
...
1.11.0 新增 API 中转站兼容性检测、100 分制脱敏公共排名和平台主页入口，并移除主界面的旧中转站推荐与项目继续板块；账号管理、本地历史数据和超大聊天安全继续能力保持不变。

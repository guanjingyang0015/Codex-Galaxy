...
- “批量检测已保存 API”会并行检测所有已保存 Key 的 API 账号，并分别显示进度、剩余时间和最终报告。
- 检测包含 `/models`、Responses 返回、模型是否列出、推理强度、响应速度和 usage 结构，并给出 100 分制的兼容性参考分。
- 评分分为模型核对 40 分、接口协议 25 分、推理强度 15 分、稳定性 10 分、速度 10 分。账号设置中的“模型 ID”就是期望型号；如果期望 `gpt-6`，响应声明为 `gpt-5.6-sol` 或 `gpt-5.5`，会明确显示模型不匹配，总分最高 49 分。响应不声明模型时只会显示“无法确认”，不会猜测是真实型号。
...
- “API 排名”按综合分数或最近测试时间排序。点击平台主页图标会打开服务商主页；排名只代表公开测试表现，不能证明其一定连接官方上游。
...
1.12.0 新增多 API 并行检测、模型期望值与响应声明值核对、分项通过/失败证据和模型不匹配总分上限；排名分数显示“分”，每个排名站点都可点击访问其已验证主页或 API 域名。

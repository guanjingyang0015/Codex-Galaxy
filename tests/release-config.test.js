...
test("release identity stays compatible with 0.1.0 upgrades and preserves user data", async () => {
  ...
  assert.doesNotMatch(html, /id="currentProfileName"/);
  assert.doesNotMatch(renderer, /\$\("#currentProfileName"\)/);
  ...
  assert.match(html, new RegExp(`id="releaseRecordVersion">v${packageJson.version.replaceAll(".", "\\.")}<`));
  assert.match(html, /id="topupBtn"/);
  assert.match(html, /https:\/\/api\.vx314490015\.cn\/topup\//);
  assert.doesNotMatch(html, /shop\.zygtoken\.com|章鱼哥/);
  ...
  assert.match(html, /id="rankingsList"/);
  assert.doesNotMatch(html, /id="(?:rankingsDialog|rankingPreview|topRankingsBtn|rankingsBtn|statusModel|auditBtn)"/);
  assert.match(html, /id="activeTasks"/);
  assert.match(preload, /reorderProfiles/);
  assert.match(preload, /openActiveTask/);
  ...
  assert.match(styles, /@keyframes topup-pulse/);
